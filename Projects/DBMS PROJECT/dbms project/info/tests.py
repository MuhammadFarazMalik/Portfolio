...
class InfoTest(TestCase):
    ...
    def create_class(self, id='CS5A', sem=5, section='A', dept=None):
        if not dept:
    ...
    def test_usn_generation(self):
        dept = self.create_dept(id='CP', name='Computer Science')
        class_obj = self.create_class(id='CS5A', sem=5, section='A')
        student1 = Student.objects.create(class_id=class_obj, name='John Doe')
        student2 = Student.objects.create(class_id=class_obj, name='Jane Doe')
        self.assertEqual(student1.USN, '22-cp-01')  # Assuming the current year is 2022
        self.assertEqual(student2.USN, '22-cp-02')

    def test_usn_generation_department_specific(self):
        cs_dept = self.create_dept(id='CS', name='Computer Science')
        is_dept = self.create_dept(id='IS', name='Information Science')
        cs_class = self.create_class(id='CS5A', sem=5, section='A')
        is_class = self.create_class(id='IS5A', sem=5, section='A', dept=is_dept)

        student1 = Student.objects.create(class_id=cs_class, name='John Doe')
        student2 = Student.objects.create(class_id=cs_class, name='Jane Doe')
        student3 = Student.objects.create(class_id=is_class, name='Alice Smith')

        self.assertEqual(student1.USN, '22-cs-01')  # Assuming the current year is 2022
        self.assertEqual(student2.USN, '22-cs-02')
        self.assertEqual(student3.USN, '22-is-01')  # Independent serialization for IS department
